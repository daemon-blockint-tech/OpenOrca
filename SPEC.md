...
T5|x|ArgoCD machine account openorca + RBAC + lib client REST — DONE 2026-08-22, `packages/argocd` 7/7 test + live (can-i: openorca/* yes, default/* no) (OO-003)|V8,I.api
T6|x|tools argocd_* + rollout_* dgn HITL — DONE 2026-08-22, 6 tool + INTERRUPT_ON; drill recall→promote LULUS live (OO-003)|V1,V4,V5,V9,I.tools
T7|x|webhook receiver: notifications → spawn agent Detect — DONE 2026-08-22, `apps/api` 8/8 test + live smoke di kind (401 tanpa secret, 202+spawn Healthy/Degraded-non-aborted, V6 gate fail-open terverifikasi); spawn point = injectable `onEvent`, stub log sampai T8|I.api,kit-fleet
T8|~|Hunt stage: subagents paralel per kelas vuln — IMPLEMENTED 2026-08-22, `packages/agents/src/hunt/*` (6 spesialis ter-scope V13, responseFormat findings, persist deterministik V7) + prasyarat minimal: model registry V17 (9 provider, `resolveModel`) & sandbox registry V18 (`DockerGvisorSandbox`, hardening flags enforced). 28/28 test unit; live wiring terverifikasi s/d gate sandbox (fail-loud tanpa gVisor). E2E penuh butuh LLM key + gVisor/runsc — belum didrill|V9,V11,V13,kit-workflow
T9|x|Validate: fn blast + owner-of → route finding tervalidasi ke owner — DONE 2026-08-22, `triage/validate.ts`; tolak eksplisit (suppressed/duplicate/no-owner), 11 test live vs schema asli|V6,V12,kit-ontology,kit-workflow
T10|x|ApplicationSet openorca-fleet + AppProject + notifications config — DONE 2026-08-22, live di kind (cluster di-recreate bersih via dev-up.sh): AC1 (demo-openorca-kind ter-generate dr envs/prod/*, label managed+service benar, Rollout Healthy), AC2 (hapus dir checkout → Application ter-prune), AC3 (sync → notifications POST webhook receiver :8899, event terparse dgn join-key service=demo). Manifest reproducible di `deploy/argocd/` (appproject, applicationset, cluster-label, machine-account+RBAC B11, notifications) + fleet repo diisi envs/prod/demo (canary setWeight:50+pause:{}). Temuan: AppProject wajib whitelist Namespace utk CreateNamespace=true; in-cluster butuh cluster secret ber-label openorca.io/fleet utk generator selector|V10,kit-fleet
T11|x|Learn: verdict + adjudikasi + remediation → graph; run berikutnya suppress dismissed — DONE 2026-08-22, `triage/learn.ts`; siklus V15 penuh diuji (dismiss → finding baru path sama tersuppress)|V7,V15,kit-ontology
T12|.|e2e drill: detect→recall→rollforward di demo app|V1,V4,V5,V6
T13|x|Foundation stage: map repo/deps/interfaces/auth/deploy → threat model + attack surface → graph — DONE 2026-08-22, `hunt/foundation.ts`; LLM mengembalikan ThreatModel terstruktur, pipeline yang persist (⊥ LLM menulis TypeQL); gate V11 fail-closed thd artefak korup. 7 test live|V7,V11,kit-workflow
T14|~|Challenge+dedup — dedup source→sink DETERMINISTIK SELESAI 2026-08-22 (`triage/dedup.ts`, idempotent, 2 test live). Judge independen (V12) BELUM: butuh LLM key utk drill|V12,V14,kit-workflow,kit-ontology
T15|x|Correlate+report: collapse kelemahan terkait → unified finding — DONE 2026-08-22, `triage/report.ts`; severity unified = MAX kelompok (collapse ⊥ menurunkan urgensi), CWE digabung|V14,kit-workflow,kit-ontology
T16|x|Cross-model harness: repeated runs + model comparison — DONE 2026-08-22, `harness/crossmodel.ts`; exclusivePaths + bestSingleModelCoverage + konsistensi antar-repetisi; refusal/error/dup dihitung TERPISAH. 8 test (runFn injectable, ⊥ butuh API key)|V16,kit-workflow
T17|~|Shared engineer review surface — API SELESAI 2026-08-22 (`apps/api/src/review.ts`: list unified, detail+evidence+routing, severity/owner/note/verdict, suppressions; 10 test live). UI web `apps/web` BELUM dibangun; auth masih shared-secret (⊥ identitas per-engineer)|V1,V12,kit-workflow
T18|~|Scaffold monorepo per plan-project-structure — pnpm workspace + packages/{ontology,agents,argocd} + apps/api live; apps/web + packages/shared belum|V9,plan-project-structure
...
B9|2026-08-22|binding grpc-gateway ArgoCD ⊥ seragam; error-nya menyesatkan (500 `required field "kind" not set` / 400 `cannot unmarshal object into Go value of type string`)|verifikasi ke `application.proto` v3.5.1: `RunResourceActionV2` = `body:"*"` → SELURUH message di body, ⊥ query param; `PatchResource` → resource di QUERY (`resourceName`, bukan `name`) + body = JSON string telanjang (field `patch` bertipe string). Client + smoke diperbaiki, drill lolos
B10|2026-08-22|deteksi canary mid-flight pakai health `Progressing`/`Paused` → timeout; ArgoCD melaporkan canary yg parkir di step `pause` sbg **`Suspended`** (message CanaryPauseStep)|waitFor terima `Suspended`/`Progressing`; `AppStatus` dapat field baru `rolloutPaused` — terverifikasi live via drill
B11|2026-08-22|policy.csv RBAC (kit-fleet) tanpa bentuk subresource → `PatchResource` & semua resource action (`abort`/`retry`/`promote-full`) 403 walau verb polos di-allow; log server: "tried to action/argoproj.io/Rollout/promote-full"|tambah rule `update/argoproj.io/*/*/*` + `action/argoproj.io/*/*`; diterapkan live — kit-fleet.md §Machine account diperbarui
B13|2026-08-22|subquery list TypeQL yang me-`return` >1 variabel ditolak (FER13 "non-scalar non-reduce result")|multi-field harus pakai `fetch` bersarang di dalam list: `[ match ...; fetch { "a": $a, "b": $b }; ]` — ditemukan saat T17, diverifikasi live
B12|2026-08-22|smoke-recall flaky: (a) patch live-image langsung di-revert autosync; (b) patch di tengah operasi sync ditimpa manifest git 1s kemudian (event: rev N→N+1 SkipSteps); (c) bump image sama = no-op; (d) reset tanpa sync → promote-full mempromosikan revisi lama|urutan wajib: retry → sync → wait parked/healthy → wait operation settled+Synced → disable autosync → patch image BERBEDA dari current → uji → restore autosync
B13|2026-08-22|ApplicationSet dgn `ServerSideApply=true` → app OutOfSync permanen walau Sync-Succeeded & Healthy: diff SSA bocor managedFields manager lain + defaults kube-apiserver (protocol:TCP); ignoreDifferences + managedFieldsManagers ⊥ menutupnya|buang ServerSideApply dr syncOptions (CSA normalisasi matang utk manifest service-scale); kit-fleet §ApplicationSet dikoreksi; terverifikasi live: Synced Healthy stabil 3x cek @15s
```
